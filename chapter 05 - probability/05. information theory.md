...
1. Compute cross-entropy and KL divergence between a true distribution and several approximations. Verify that $D_{\text{KL}}(p \| q) = H(p, q) - H(p)$.
...
1. Show that KL divergence is not symmetric by computing $D_{\text{KL}}(p \| q)$ and $D_{\text{KL}}(q \| p)$ for two different distributions.
...
1. Simulate cross-entropy loss during training. Create a "true" one-hot label and show how the loss decreases as the model's predicted probabilities improve.
