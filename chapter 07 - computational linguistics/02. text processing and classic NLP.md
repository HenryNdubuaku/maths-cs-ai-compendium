...
1. Implement BPE tokenisation from scratch. Start with character-level tokens and iteratively merge the most frequent pairs.
...
1. Build a bigram language model and compute perplexity on a test sentence. Experiment with Laplace smoothing.
...
1. Implement TF-IDF from scratch and use cosine similarity to find the most similar document to a query.
