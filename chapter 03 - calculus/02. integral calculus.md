...
1. Verify the Fundamental Theorem of Calculus numerically. Define $F(x) = \int_0^x t^2\, dt = \frac{x^3}{3}$ and check that its derivative (computed via `jax.grad`) equals $x^2$.
...
1. Visualise the area under $f(x) = \sin(x)$ from $0$ to $\pi$. Use `plt.fill_between` to shade the area and compute it numerically with a Riemann sum.
