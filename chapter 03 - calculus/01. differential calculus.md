...
1. Use JAX's automatic differentiation to compute the derivative of $f(x) = x^3 - 2x + 1$ at several points. Compare with the analytical derivative $f'(x) = 3x^2 - 2$.
...
1. Verify the chain rule numerically. Define $f(x) = \sin(x^2)$, compute its derivative via `jax.grad`, and compare with the analytical result $2x\cos(x^2)$.
...
1. Visualise the derivative. Plot $f(x) = x^3 - 3x$ and its derivative $f'(x) = 3x^2 - 3$ on the same graph. Notice where $f'(x) = 0$ corresponds to the peaks and valleys of $f$.
