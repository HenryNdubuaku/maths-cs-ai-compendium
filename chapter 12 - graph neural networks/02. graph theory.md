...
1. Compute the graph Laplacian and its eigenvalues. Verify that the smallest eigenvalue is 0 and the corresponding eigenvector is constant.
...
1. Perform spectral clustering on a graph with two communities. Embed nodes using the Fiedler vector and separate them by sign.
