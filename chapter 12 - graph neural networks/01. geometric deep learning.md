...
1. Verify permutation invariance of DeepSets-style aggregation. Apply a shared function to each element of a set, sum the results, and check that the output is the same regardless of element ordering.
...
1. Explore group structure. Verify that 2D rotation matrices form a group by checking closure, associativity, identity, and inverse.
