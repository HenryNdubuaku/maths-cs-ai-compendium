...
1. Implement NEON ReLU and softmax-max-finding. Practice the load→compute→store pattern with different operations.
...
1. Compare auto-vectorised code against hand-written NEON intrinsics. Compile with `-fopt-info-vec` (GCC) or `-Rpass=loop-vectorize` (Clang) to see what the compiler does.
